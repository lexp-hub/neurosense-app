...
const App = () => {
  ...
  return (
    <div className="h-screen w-full bg-[#0b1120] text-slate-100 flex flex-col items-center overflow-hidden font-sans p-4 md:p-6">
      <Suspense fallback={<Loader2 className="animate-spin text-indigo-500 mt-20" />}>
        ...
        <div className="w-full max-w-xl flex flex-col items-center flex-1 min-h-0">
          ...
          <header className="w-full bg-[#161f3266] rounded-2xl md:rounded-[2rem] backdrop-blur-xl border border-[#ffffff1a] p-3 flex justify-between items-center shrink-0 shadow-2xl mb-6">
          ...
          <main className="w-full flex-1 flex flex-col items-center justify-center min-h-0 relative">
            
            <div className="shrink-0 w-24 h-24 md:w-32 md:h-32 flex items-center justify-center relative mb-6 transition-all">
            ...
            {!gameStarted ? (
              ...
              <div className={`w-full flex flex-col bg-[#161f32]/80 backdrop-blur-2xl border border-[#ffffff1a] p-6 rounded-[2rem] shadow-3xl animate-in slide-in-from-bottom-8 ${loading ? 'opacity-50 pointer-events-none' : ''}`}>
                    ...
                    {current?.gameOver ? (
                      ...
                      <>
                        <div className="grid grid-cols-2 gap-3">
                          <button onClick={() => nextStep("Sì")} className="bg-[#4f46e5] p-4 rounded-xl font-bold text-lg active:scale-95 hover:bg-indigo-500 transition-all">Sì</button>
          ...
          <div className="fixed inset-0 bg-black/80 backdrop-blur-md flex items-center justify-center p-6 z-[100] animate-in fade-in">
            <div className="bg-[#111827] border border-[#ffffff1a] w-full max-w-sm rounded-[2rem] p-6 shadow-2xl">
              ...
              <div className="space-y-4">
                <div className="bg-[#1f2937] rounded-lg p-3 border border-[#ffffff1a] text-slate-300 text-[9px] truncate">{config.baseUrl}</div>
                <div className="bg-[#1f2937] rounded-lg p-3 border border-indigo-500/30 text-slate-200 text-[9px] font-mono break-all">{config.model}</div>
                <button onClick={() => setShowSettings(false)} className="w-full bg-[#4f46e5] p-3 rounded-lg font-bold text-[9px] uppercase hover:bg-indigo-500 transition-all">Chiudi</button>
              </div>
            </div>
          </div>
        )}
